...
const useStubTransport = process.env.NODE_ENV == 'test' || process.env.MAILER_DISABLED;
const useAws = config.mailer.transport === 'aws';

const transportEngine = useStubTransport ? stubTransport() : useAws ? new SesTransport({
    ses: new AWS.SES(),
    rateLimit: 50
}) : new SMTPTransport({
    service: 'Gmail',
...
transprot.use('compile', htmlToText());
